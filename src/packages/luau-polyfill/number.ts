...
namespace Number {
	...
	export function toExponential(value: number | string, fractionDigits?: number) {
		...
		if (fractionDigits !== undefined) {
			if (!typeIs(fractionDigits, "number")) throw "TypeError: fractionDigits must be a number between 0 and 100";

			if (fractionDigits < 0 || fractionDigits > 100)
				throw "RangeError: fractionDigits must be between 0 and 100";
		}
